...
const setRevokeOnToken = `-- name: SetRevokeOnToken :one
...
func (q *Queries) SetRevokeOnToken(ctx context.Context, token string) (RefreshToken, error) {
	row := q.db.QueryRowContext(ctx, setRevokeOnToken, token)
	var i RefreshToken
	err := row.Scan(
		&i.Token,
		&i.CreatedAt,
